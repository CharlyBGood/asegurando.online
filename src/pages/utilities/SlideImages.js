const Slides = [
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1741811429/buildings_usntuj.png',
    alt: 'Edificios modernos en la ciudad',
    text: 'La CALIDAD de servicio se mide con la SATISFACCIÓN de nuestros ASEGURADOS.',
  },
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1743995894/coolboat_apqo4l.jpg',
    alt: 'Yate de lujo en el mar',
    text: 'Embarcaciones: el placer de sentirse siempre ACOMPAÑADO.',
  },
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1743995892/coolcar_lzcb2o.jpg',
    alt: 'Auto de lujo en la carretera',
    text: 'Tenemos las MEJORES bonificaciones del mercado.',
  },
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1743995893/cooldog_uhoxkw.jpg',
    alt: 'Perro descansando en el sofá',
    text: 'Sabemos cuidar a tu MEJOR amigo. ¡Como se lo MERECE!',
  },
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1743995894/coolhouse_gxvx1q.jpg',
    alt: 'Casa de lujo moderna',
    text: 'SOLUCIONES a la medida de tus necesidades.',
  },
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1743995895/coolmotogirl_j23jlf.jpg',
    alt: 'Mujer andando en moto en la carretera',
    text: 'Disfrutá el CAMINO, con una mejor opción.',
  },
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1744681429/coolworkers_edxu46.jpg',
    alt: 'Grupo de trabajo con casco de construcción y dos brazos estrechando la mano',
    text: 'Trabaja en confianza con la mejor PROTECCIÓN.',
  },
  {
    image: 'https://res.cloudinary.com/dr8pwzxzn/image/upload/v1743995823/hombre-joven-tiro-completo-sentado-scooter_23-2149356786_ai6q0s.jpg',
    alt: 'Hombre sentado con un monopatín',
    text: 'Manejate con TRANQUILIDAD por la ciudad.',
  },

];

export default Slides;